...
from crud import (
    ...
    create_medical_history, get_user_medical_history, create_user
)
...
# --- Login ---
@app.post("/login")
def login(email: str, password: str, cnp: str):
    try:
        user = get_user_by_email(email)
        if not user or user.password != password or user.cnp != cnp:
